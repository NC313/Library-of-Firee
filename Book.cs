namespace Library_of_Fire
{
    public class Book
    {
        ...
        public static string TryGetAppDirectory()
        {
            string LibraryTerminalPath = @"c:\LibraryTerminal\";
            if (!Directory.Exists(LibraryTerminalPath))
            {
                Directory.CreateDirectory(LibraryTerminalPath);
            }
            return LibraryTerminalPath;
        }
        public static void SaveBookList()
        {
            string fullPath = $"{TryGetAppDirectory()}BookListDownload.csv";
